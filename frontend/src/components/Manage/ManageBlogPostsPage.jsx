...
const BlogPostsLoadingSkeleton = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...Array(3)].map((_, i) => (
            <Card key={i} className="flex flex-col"><Skeleton className="w-full h-48 rounded-t-lg" /><CardHeader className="pt-4"><Skeleton className="h-6 w-3/4 mb-1" /><Skeleton className="h-4 w-1/2" /></CardHeader></Card>
        ))}
    </div>
);
        ...
        <div className="space-y-6 p-4 md:p-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <Button className="flex items-center gap-2" onClick={() => { setCurrentPost(null); setIsModalOpen(true); }}>
            ...
            {isLoading ? <BlogPostsLoadingSkeleton />
             ...
             : (
                <>
                    <BlogPostsList posts={posts} onEditPost={(post) => { setCurrentPost(post); setIsModalOpen(true); }} onDeletePost={handleDeletePost} />
